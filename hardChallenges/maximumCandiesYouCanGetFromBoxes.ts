// First Approach - Queueing initial boxes and iterating over queue, adding more boxes to queue by contained
// boxes, adding to the answer the candies if the box have status "opened". (25ms - Beats -%)
function maxCandies(status: number[], candies: number[], keys: number[][], containedBoxes: number[][], initialBoxes: number[]): number {
  const openedBoxes: Set<number> = new Set(), boxes: Set<number> = new Set(initialBoxes), queue: number[] = [...initialBoxes];
  let ans = 0;
  while(queue.length > 0) {
    const box = queue.pop()!;

    if(status[box] === 0 || openedBoxes.has(box) || !boxes.has(box))
      continue;
    ...
    for(const boxIndex of containedBoxes[box]) {
...
// Second Approach - Same logic of first approach but with optimized queue controlling. (16ms - Beats 66.67%)
function maxCandies(status: number[], candies: number[], keys: number[][], containedBoxes: number[][], initialBoxes: number[]): number {
  const closedBoxes: Set<number> = new Set(), openedBoxes: Set<number> = new Set(), queue: number[] = [...initialBoxes];
  let ans = 0;
  while(queue.length > 0) {
    const box = queue.pop()!;
    ...
    for(const key of keys[box]) {
      if(closedBoxes.has(key) && !openedBoxes.has(key)) {
        queue.push(key);
      }
